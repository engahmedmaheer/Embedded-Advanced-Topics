...
void Reset_Handler()
{
	...
	/*Get Size*/
	uint32_t LocalSize = (uint32_t)(&_edata) -(uint32_t)(&_sdata);
	
	uint8_t* SrcPtr = (uint8_t*)&_eText; /*Start Of .Data Section in flash*/
	
	uint8_t* DesPtr = (uint8_t*)&_sdata; /*Start Of .Data Section in SRAM*/
	
	uint32_t LocalCounter ;
	
	for(LocalCounter = 0 ; LocalCounter <LocalSize ; LocalCounter++)
	{
		*DesPtr = *SrcPtr ; //*DesPtr++ = *SrcPtr++
		SrcPtr++;
		DesPtr++;
	}
	
	/*2-init .bss section in sram to zero*/
	
	LocalSize = (uint32_t)&_ebss - (uint32_t)&_sbss ;
	
	DesPtr = (uint8_t*)&_sbss ;
